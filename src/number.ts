...
const gcdCalculate = (firstNumber:any, secondNumber:any):any => 
 ...
 * @param {*} number
 */
const getFactorial = (number:any):any => {
 ...
 * @param {*} number
 */
const bigPower = (base:any, power:any):any => {
  if (power === 0) return 1;
  if (power % 2 === 0) {
 ...
 */
const pascalTriangle = (lineNumber:any) => {
  let currentLine = [1],
 ...
 */
const getPrimeList = (number:any) => {
  let isPrime = new Array(number + 1),
    primes = [];
  isPrime[0] = true;
  isPrime[1] = true;
...
const reverseNumber = (number:any) => {
  let sign = false;
  if (number < 0) { sign = true; number *= -1; }
